...
test("inferPartyLabel: generates accurate labels for various customer combinations", () => {
  assert.equal(
    inferPartyLabel("John Doe", 2, 0),
    "John Doe & Party (2 Adults)",
  );
  assert.equal(
    inferPartyLabel("Family Smith", 2, 2),
    "Family Smith & Party (2 Adults, 2 Children)",
  );
...
test("inferGreetingName: extracts clean greeting from customer name", () => {
  ...
  assert.equal(
    inferGreetingName("Dr. Jane Smith & Family"),
    "Dear Dr. Jane Smith & Family",
  );
...
test("validateHotelDates: validates check-in/out boundaries correctly", () => {
  const valid = validateHotelDates(
    "2026-10-01",
    "2026-10-05",
    "2026-10-01",
    "2026-10-10",
  ...
  const invalidOrder = validateHotelDates(
    "2026-10-05",
    "2026-10-01",
    "2026-10-01",
    "2026-10-10",
  ...
  const outOfTrip = validateHotelDates(
    "2026-09-28",
    "2026-10-05",
    "2026-10-01",
    "2026-10-10",
...
test("buildInitialFactsFromRequest: maps Lead Request fields into valid QuotationFacts", () => {
  ...
  const facts = buildInitialFactsFromRequest(
    mockRequest,
    ensureFactsDefaults(emptyFacts()),
  );
...
import {
  deriveDayWithStays,
  syncRouteTableToFacts,
} from "../components/quotation-workspace/useRouteTableSync.ts";
...
test("deriveDayWithStays & syncRouteTableToFacts: roundtrip conversion", () => {
  ...
  assert.equal(
    syncedFacts.service_facts.hotels.some((h) => h.name === "Paradise Cruise"),
    true,
  );
});
...
test("promptOptionsCatalog: contains required structured entries", () => {
  ...
  assert.equal(
    BRAND_OPTIONS.some((b) => b.id === "capella_travel"),
    true,
  );
  assert.equal(
    BRAND_OPTIONS.some((b) => b.id === "selvara"),
    true,
  );
  assert.equal(
    BRAND_OPTIONS.some((b) => b.id === "vietnam_safar"),
    true,
  );

  assert.equal(MODE_OPTIONS.length >= 2, true);
  assert.equal(
    MODE_OPTIONS.some((m) => m.id === "storytelling"),
    true,
  );
  assert.equal(
    MODE_OPTIONS.some((m) => m.id === "detailed"),
    true,
  );

  assert.equal(GROUND_RULE_OPTIONS.length >= 5, true);
  assert.equal(
    GROUND_RULE_OPTIONS.some((g) => g.id === "GR-7030"),
    true,
  );
  assert.equal(
    GROUND_RULE_OPTIONS.some((g) => g.id === "GR-DAY-NAMING"),
    true,
  );

  assert.equal(CONSTRAINT_OPTIONS.length >= 3, true);
  assert.equal(
    CONSTRAINT_OPTIONS.some((c) => c.id === "schema_validation"),
    true,
  );
});
...
test("CATEGORIES: contains the eight Product Catalog tabs with complete metadata", () => {
...
test("PRODUCT_CATEGORY_BY_SLOT: covers every backend product category exactly once", () => {
...
// ==========================================
console.log(
  "🗓️  7. Testing Itinerary Day Standardization (Use Cases 1, 2, 3)...",
);
  ...
  const intakeDays = [
    {
      day_number: 1,
      destination: "Hanoi",
      accommodation_id: "acc-metropole",
    ...
    },
    {
      day_number: 4,
      destination: "Halong Bay",
      accommodation_id: "acc-heritage-cruise",
      accommodation_name: "Heritage Line Cruise",
      room_type: "Captain Suite",
      summary: "Boarding cruise",
    },
    {
      day_number: 5,
      destination: "Hanoi",
...
test("UseCase 3: Bi-directional sync & hydration between Itinerary Days and Hotels", () => {
      ...
      itinerary: [
        {
          day_number: 1,
          destination: "Hanoi",
          summary: "Day 1 in Hanoi",
          meals: ["Breakfast"],
          highlights: [],
          notes: [],
          display_date: "2026-11-10",
          overnight: "Hanoi",
        ...
        },
        {
          day_number: 2,
          destination: "Hanoi",
          summary: "Day 2 in Hanoi",
          meals: ["Breakfast"],
          highlights: [],
          notes: [],
          display_date: "2026-11-11",
          overnight: "Hanoi",
          sense_of_pace: "balanced",
        },
        {
  ...
  assert.equal(
    hydratedItinerary[0].accommodation_name,
    "Sofitel Legend Metropole",
  );
  assert.equal(
    hydratedItinerary[1].accommodation_name,
    "Sofitel Legend Metropole",
  );
  assert.equal(
    hydratedItinerary[2].accommodation_name,
  ...
  const modifiedDays = [
    {
      day_number: 1,
      destination: "Hanoi",
      accommodation_id: "acc-capella",
      accommodation_name: "Capella Hanoi",
      room_type: "Opera Suite",
      summary: "Day 1",
    },
    {
      day_number: 2,
  ...
  assert.equal(
    syncedFacts.trip_facts.itinerary[0].accommodation_name,
    "Capella Hanoi",
  );
...
test("routeRules.parseRouteTokens: accurately parses arrow, dash, comma, and newline delimiters", () => {
  const parsed1 = parseRouteTokens(
...
test("routeRules.deriveRouteFromItinerary: extracts arrivalCity, departureCity, and unique destinations in order", () => {
  ...
  assert.deepEqual(meta.destinations, [
    "Ho Chi Minh City",
    "Mekong Delta",
    "Da Nang",
    "Hanoi",
  ...
  assert.equal(
    meta.displayRouteText,
    "Ho Chi Minh City – Mekong Delta – Da Nang – Hanoi",
  );
});
...
test("tripReconciler.applyRouteSequence: expands itinerary, recalculates dates and syncs route metadata", () => {
  ...
  assert.deepEqual(updated.destinations, [
    "Ho Chi Minh City",
    "Mekong Delta",
    "Da Nang",
    "Hanoi",
