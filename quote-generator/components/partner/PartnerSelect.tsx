...
export function PartnerSelect({
  ...
  const [drawerMode, setDrawerMode] = useState<PartnerDrawerMode>(null);

  const { items, isLoading, mutate } = usePartnerSearch(query, {
    active: "true",
    initialSelectedId: value,
  });

  const activePartners = items.filter((p) => p.is_active);
  const selectedPartner = items.find((p) => p.id === value) ?? null;
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    switch (e.key) {
      case "ArrowDown":
        e.preventDefault();
        setHighlightedIndex((prev) => (prev < activePartners.length - 1 ? prev + 1 : prev));
        break;
      case "ArrowUp":
        e.preventDefault();
      ...
      case "Enter":
        e.preventDefault();
        if (activePartners[highlightedIndex]) {
          handleSelect(activePartners[highlightedIndex]);
        }
        break;
      case "Escape":
        e.preventDefault();
  ...
  return (
    <div ref={containerRef} className={cn("relative flex flex-col gap-1.5", className)}>
        ...
        {allowManage ? (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className={cn(
                getTypographyClassName("caption"),
                "text-[var(--color-muted)] hover:text-[var(--color-on-surface)] flex items-center gap-0.5 cursor-pointer disabled:opacity-50"
              )}
            >
      ...
      <button
        ...
        aria-label={ariaLabel || label || "Select Partner"}
      ...
      >
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          <Building2 size={16} className="shrink-0 text-[var(--color-accent)]" aria-hidden="true" />
          {selectedPartner ? (
            <div className="flex flex-col min-w-0">
              <span className={cn(getTypographyClassName("bodySm"), "truncate text-[var(--color-on-surface)]")}>
                {selectedPartner.company_name}
              </span>
              {selectedPartner.contact_name || selectedPartner.market ? (
                <span className={cn(getTypographyClassName("caption"), "truncate text-[var(--color-muted)]")}>
                  {[selectedPartner.contact_name, selectedPartner.market].filter(Boolean).join(" · ")}
                </span>
              ) : null}
            </div>
          ) : (
            <span className={cn(getTypographyClassName("bodySm"), "text-[var(--color-muted)] truncate")}>
              {placeholder}
            </span>
          )}
        </div>
        ...
        >
          ...
          <div className="relative mb-2">
            ...
            <input
              ...
              onKeyDown={handleKeyDown}
              placeholder="Search agency name, contact or email..."
              className={cn(
          ...
          >
            {isLoading ? (
              <div className="flex items-center justify-center gap-2 py-6 text-[var(--color-muted)]">
                ...
                <span className={cn(getTypographyClassName("caption"))}>Loading partners...</span>
              </div>
            ) : activePartners.length === 0 ? (
              <div className="py-6 text-center text-[var(--color-muted)]">
                <Building2 size={24} className="mx-auto mb-1 opacity-40" aria-hidden="true" />
                <p className={cn(getTypographyClassName("bodySm"))}>No partners match &quot;{query}&quot;</p>
                {allowManage ? (
                  <button
                    type="button"
                    onClick={() => {
                ...
                return (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span className="block truncate">{partner.company_name}</span>
