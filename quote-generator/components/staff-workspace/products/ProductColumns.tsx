  ...
  return [
    {
      ...
      render: (profile) => (
        <div className="flex flex-col gap-1">
          <h4 className={cn(getTypographyClassName("cardTitle"), "text-[var(--color-on-surface)]")}>{profile.title}</h4>
    ...
    },
    {
      key: "destination",
      header: "Destination",
      render: (profile) => <span className={cn(getTypographyClassName("bodySm"))}>{profile.destination_id}</span>,
    },
    {
      key: "supplier",
      header: "Supplier",
      render: (profile) =>
        profile.supplier_id ? (
          <span className={cn(getTypographyClassName("bodySm"))}>{profile.supplier_id}</span>
        ) : (
          <span className={cn(getTypographyClassName("caption"), "text-amber-700")}>⚠ chưa gắn NCC</span>
        ),
    },
    {
      key: "unit",
      header: "Unit × Time basis",
      render: (profile) => (
        <span className={cn(getTypographyClassName("bodySm"), "capitalize")}>
