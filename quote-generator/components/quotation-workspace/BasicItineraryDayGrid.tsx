...
export default function BasicItineraryDayGrid({ days, onChange }: Props) {
  ...
  const handleRemoveDay = (index: number) => {
    const updated = days.filter((_, i) => i !== index).map((day, i) => ({ ...day, day_number: i + 1 }));
    onChange(updated);
  };
