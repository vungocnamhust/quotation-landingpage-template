...
export default function ContextualInspector({ selected, resolvedHandoff, renderedValue, onSave, onHandoff }: {
  ...
  return <aside className="grid content-start gap-4 rounded-[var(--radius-card)] border border-[var(--color-border)] bg-[var(--color-surface)] p-4">
    ...
    {!selected ? null : <><h2 className={cn(getTypographyClassName('cardTitle'), 'text-[var(--color-on-surface)]')}>{selected.fieldId.replaceAll('.', ' · ')}</h2>
      {owner === 'design' && control !== 'none' ? <DesignControl key={`${selected.fieldId}:${renderedValue}`} initialValue={renderedValue} control={control === 'textarea' ? 'textarea' : 'text'} pending={pending} onSave={(value) => startTransition(async () => { try { await onSave(selected, value); setNotice('Design override saved.'); } catch (error) { setNotice(error instanceof Error ? error.message : 'Design override could not be saved.'); } })} /> : <><p className={cn(getTypographyClassName('bodySm'), 'text-[var(--color-muted)]')}>{owner === 'system' ? 'System copy has no quotation-level editor.' : `Canonical source: ${resolvedHandoff?.source ?? selected.source}.`}</p>{resolvedHandoff ? <button type="button" onClick={() => onHandoff(resolvedHandoff)} className={cn(getTypographyClassName('buttonSecondary'), 'w-fit rounded-[var(--radius-button)] border border-[var(--color-border)] px-4 py-2')}>Open {resolvedHandoff.stage === 'facts' ? 'Facts' : 'Content Studio'}</button> : null}</>}
    </>}
  </aside>;
}

function DesignControl({ initialValue, control, pending, onSave }: { initialValue: string; control: 'text' | 'textarea'; pending: boolean; onSave: (value: string) => void }) {
  const [value, setValue] = useState(initialValue);
  return <><label className="grid gap-2"><span className={cn(getTypographyClassName('label'), 'text-[var(--color-muted)]')}>Presentation copy</span>
    {control === 'textarea' ? <textarea value={value} onChange={(event) => setValue(event.target.value)} className={cn(getTypographyClassName('bodySm'), 'min-h-28 rounded-[var(--radius-card)] border border-[var(--color-border)] p-3')} /> : <input value={value} onChange={(event) => setValue(event.target.value)} className={cn(getTypographyClassName('bodySm'), 'min-h-11 rounded-[var(--radius-button)] border border-[var(--color-border)] px-3')} />}
  </label><button type="button" disabled={pending || !value.trim()} onClick={() => onSave(value.trim())} className={cn(getTypographyClassName('buttonPrimary'), 'w-fit rounded-[var(--radius-button)] bg-[var(--color-action-primary-surface)] px-4 py-2 text-[var(--color-action-primary-text)] disabled:opacity-50')}>Save</button></>;
}
