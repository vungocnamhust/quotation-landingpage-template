...
export type ContentEditorField = { id: string; label: string; path: Array<string | number>; control: 'input' | 'textarea' | 'string-list' | 'hotel-editorial-list'; required: boolean; minLength: number; maxLength: number };
export type ContentFactInput = { id: string; label: string; path: Array<string | number>; required: boolean };
export type ContentEditor = { owner: 'fact' | 'fact-derived' | 'content' | 'design'; generation: boolean; automationPolicy?: 'manual' | 'auto' | 'bypass'; candidateContract?: 'generated' | 'hotel-editorial' | 'pricing-framing'; editor: 'narrative' | 'fact-preview' | 'checklist'; recipeVersion: string; schemaVersion: string; fields: ContentEditorField[]; factInputs: ContentFactInput[]; defaultInstructions: { storytelling: string; detailed: string } | null };
