...
function emptyHotel(): HotelFact {
  return { accommodation_id: null, destination: null, destination_ref: null, name: null, room_type: null, check_in: null, check_out: null, intro: "Breakfast included.", phone: null, display_city: null, display_date: null, hotel_asset: null, room_asset: null };
}

function hotelFromProfile(profile: AccommodationProfile): HotelFact {
  return { accommodation_id: profile.id, destination: profile.destination, destination_ref: profile.destination_ref, name: profile.name, room_type: profile.room_type, check_in: profile.check_in, check_out: profile.check_out, intro: profile.intro || "Breakfast included.", phone: profile.phone, display_city: profile.display_city, display_date: profile.display_date, hotel_asset: profile.hotel_asset, room_asset: profile.room_asset };
}

function Field({ label, value, onChange, type = "text", required = false, min, max }: { label: string; value: string | number | null; onChange: (value: string) => void; type?: string; required?: boolean; min?: string | number; max?: string | number }) {
  return <label className="flex flex-col gap-2"><span className={cn(getTypographyClassName("label"), "flex justify-between gap-3 text-[var(--color-muted)]")}><span>{label}</span><span className={cn(getTypographyClassName("caption"), required ? "text-[var(--color-accent)]" : "text-[var(--color-muted)]")}>{required ? "Required" : "Optional"}</span></span><input className={inputClass} type={type} min={min} max={max} required={required} value={value ?? ""} onChange={(event) => onChange(event.target.value)} /></label>;
}
...
export default function QuotationIntakeForm({ facts: inputFacts, options, pending = false, draftMediaSelections, onDraftMediaSelectionChange, onChange, onSubmit }: Props) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); onSubmit(); }} className="mx-auto flex w-full max-w-5xl flex-col gap-6">
    ...
    <IntakeCard title="Accommodations" description="Select the stays for this quotation. You can sync accommodation slots from your Brief Route." alternateBg={true}>
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3 mb-1">
          <span className={cn(getTypographyClassName("label"), "text-[var(--color-muted)]")}>Accommodation Slots</span>
          <button type="button" onClick={syncHotelsFromItinerary} className={cn(getTypographyClassName("buttonSecondary"), "min-h-9 rounded-[var(--radius-button)] border border-[var(--color-border)] px-3 py-1 text-[var(--color-accent)] hover:bg-[var(--color-accent-wash)] transition-colors")}>✨ Sync from Itinerary Overnights</button>
        </div>
        {facts.service_facts.hotels.map((hotel, index) => {
          const dateValidation = validateHotelDates(hotel.check_in, hotel.check_out, trip.start_date, trip.end_date);
          return <article key={index} className="grid gap-3 rounded-[var(--radius-card)] border border-[var(--color-border-strong)] bg-[var(--color-surface-muted)] p-4 shadow-2xs sm:grid-cols-2"><AccommodationPicker value={hotel.accommodation_id} name={hotel.name} destination={hotel.destination || hotel.display_city} onChange={(profile) => { patchHotel(index, profile ? hotelFromProfile(profile) : emptyHotel()); seedProfileMedia(`stays.hotels.${index}.hotelImage`, profile?.hotel_asset); seedProfileMedia(`stays.hotels.${index}.roomImage`, profile?.room_asset); }} /><Field label="Check-in" required type="date" min={trip.start_date ?? undefined} max={trip.end_date ?? undefined} value={hotel.check_in} onChange={(checkIn) => patchHotel(index, { check_in: checkIn || null })} /><Field label="Check-out" required type="date" min={hotel.check_in ?? trip.start_date ?? undefined} max={trip.end_date ?? undefined} value={hotel.check_out} onChange={(checkOut) => patchHotel(index, { check_out: checkOut || null })} /><Field label="Room type" required value={hotel.room_type} onChange={(roomType) => patchHotel(index, { room_type: roomType || null })} />{!dateValidation.valid ? <p className={cn(getTypographyClassName("caption"), "sm:col-span-2 rounded-[var(--radius-button)] border border-rose-500/30 bg-rose-500/10 p-2 text-rose-700")}>{dateValidation.message}</p> : null}<div className="sm:col-span-2 flex flex-col gap-1.5"><span className={cn(getTypographyClassName("label"), "text-[var(--color-muted)]")}>Stay notes</span><RichTextEditor value={hotel.intro ?? ""} minHeight="4rem" onChange={(introVal) => patchHotel(index, { intro: introVal || null })} /></div><MediaSlotRenderer workspace={mediaWorkspace} editorRoute="facts.services.hotel" context={{ index, destinationId: hotel.destination_ref?.id, accommodationName: hotel.name ?? undefined, profileAssetKeys: { [`stays.hotels.${index}.hotelImage`]: hotel.hotel_asset, [`stays.hotels.${index}.roomImage`]: hotel.room_asset } }} /><button type="button" onClick={() => removeHotel(index)} className={cn(getTypographyClassName("buttonSecondary"), "min-h-10 w-fit rounded-[var(--radius-button)] bg-rose-700 !text-white hover:bg-rose-800 px-3.5 shadow-2xs border border-transparent transition-all")}>Remove accommodation</button></article>;
    ...
    <IntakeCard title="Pricing options" description="Add a price per traveler and group total for each option. You can complete pricing later in Facts." alternateBg={false}>
      <div className="flex flex-col gap-3"><div className="flex items-center justify-between gap-3"><span className={cn(getTypographyClassName("label"), "text-[var(--color-muted)]")}>{`Options (${pricing.options.length}/${MAX_COMMERCIAL_OPTIONS})`}</span><button type="button" disabled={pricing.options.length >= MAX_COMMERCIAL_OPTIONS} onClick={addPricingOption} className={cn(getTypographyClassName("buttonSecondary"), "min-h-10 rounded-[var(--radius-button)] bg-[var(--color-accent)] !text-white hover:bg-[color-mix(in_srgb,var(--color-accent)_85%,black)] px-3.5 shadow-2xs border border-transparent transition-all disabled:opacity-50")}>Add option</button></div>{pricing.options.map((option, index) => { const defaultCurr = option.currency || inferDefaultCurrency(facts.brand_id, customer.market); const expectedTotal = customer.adults && option.per_traveler_amount_minor ? inferCommercialTotal(option.per_traveler_amount_minor, customer.adults) : null; const expectedPerTraveler = customer.adults && option.group_total_amount_minor ? inferCommercialPerTraveler(option.group_total_amount_minor, customer.adults) : null; const inconsistent = expectedTotal !== null && option.group_total_amount_minor !== null && expectedTotal !== option.group_total_amount_minor; return <article key={option.id} className="grid gap-3 rounded-[var(--radius-card)] border border-[var(--color-border-strong)] bg-[var(--color-surface-muted)] p-4 shadow-2xs sm:grid-cols-2"><Field label="Option label" required value={option.label} onChange={(label) => patchPricingOption(index, { label })} /><div className="flex flex-col gap-1.5"><span className={cn(getTypographyClassName("label"), "text-[var(--color-muted)]")}>Currency</span><CustomSelect value={option.currency || defaultCurr} placeholder="Select currency" options={CURRENCY_OPTIONS} onChange={(currency) => patchPricingOption(index, { currency })} /></div><Field label="Per traveler price" required type="number" value={minorAmountToInput(option.per_traveler_amount_minor, option.currency || defaultCurr)} onChange={(value) => { const perTraveler = minorAmountFromInput(value, option.currency || defaultCurr); const autoTotal = option.group_total_amount_minor === null ? inferCommercialTotal(perTraveler, customer.adults) : option.group_total_amount_minor; patchPricingOption(index, { currency: option.currency || defaultCurr, per_traveler_amount_minor: perTraveler, group_total_amount_minor: autoTotal }); }} /><Field label="Group total price" required type="number" value={minorAmountToInput(option.group_total_amount_minor, option.currency || defaultCurr)} onChange={(value) => { const groupTotal = minorAmountFromInput(value, option.currency || defaultCurr); const autoPerTraveler = option.per_traveler_amount_minor === null ? inferCommercialPerTraveler(groupTotal, customer.adults) : option.per_traveler_amount_minor; patchPricingOption(index, { currency: option.currency || defaultCurr, group_total_amount_minor: groupTotal, per_traveler_amount_minor: autoPerTraveler }); }} />{inconsistent ? <div className="sm:col-span-2 flex flex-col gap-2 rounded-[var(--radius-button)] border border-[var(--color-border-strong)] bg-[var(--color-surface-muted)] p-3 text-[var(--color-muted)]"><p className={cn(getTypographyClassName("caption"))}>{`For ${customer.adults} adults, the per traveler price equals ${formatMinorAmount(expectedTotal, option.currency || defaultCurr, facts.lang ?? "en")}; the entered group total is ${formatMinorAmount(option.group_total_amount_minor, option.currency || defaultCurr, facts.lang ?? "en")}.`}</p><div className="flex gap-2"><button type="button" onClick={() => patchPricingOption(index, { group_total_amount_minor: expectedTotal })} className={cn(getTypographyClassName("caption"), "px-2 py-1 rounded-[var(--radius-button)] border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-on-surface)] hover:bg-[var(--color-accent-wash)]")}>Apply calculated total ({formatMinorAmount(expectedTotal, option.currency || defaultCurr, facts.lang ?? "en")})</button>{expectedPerTraveler !== null ? <button type="button" onClick={() => patchPricingOption(index, { per_traveler_amount_minor: expectedPerTraveler })} className={cn(getTypographyClassName("caption"), "px-2 py-1 rounded-[var(--radius-button)] border border-[var(--color-border)] bg-[var(--color-surface)] text-[var(--color-on-surface)] hover:bg-[var(--color-accent-wash)]")}>Apply calculated per traveler ({formatMinorAmount(expectedPerTraveler, option.currency || defaultCurr, facts.lang ?? "en")})</button> : null}</div></div> : null}<button type="button" onClick={() => removePricingOption(index)} className={cn(getTypographyClassName("buttonSecondary"), "min-h-10 w-fit rounded-[var(--radius-button)] bg-rose-700 !text-white hover:bg-rose-800 px-3.5 shadow-2xs border border-transparent transition-all")}>Remove option</button></article>; })}</div>
    </IntakeCard>
