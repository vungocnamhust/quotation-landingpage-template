...
function checklistGroups(block: QuoteRecord): QuoteRecord[] { return recordList(block.groups); }
...
export function buildDisplayDocumentFromQuoteDocument({ document, brandProfile, lang, viewMode }: {
  ...
  const finalizationGroups = checklistGroups(contentBlock(contentBlocks(contentSections, 'finalization'), 'checklistGroups'));
