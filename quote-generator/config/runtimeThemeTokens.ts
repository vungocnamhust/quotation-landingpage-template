...
function assertContrast(foreground: ColorReference, background: ColorReference, palette: BrandColorPalette, label: string, minimum: number) {
  if (foreground === 'transparent' || background === 'transparent') return;
...
function mergeRecipe(recipe: ColorScopeRecipe, viewMode: ViewMode): ColorScopeRecipe {
  ...
  return adjustment ? { ...recipe, ...adjustment, viewModeAdjustments: recipe.viewModeAdjustments } : recipe;
}

function resolveScope({ id, palette, recipe, viewMode, tokens }: {
  id: ResolvedColorScope['id']; palette: BrandColorPalette; recipe: ColorScopeRecipe; viewMode: ViewMode; tokens: BrandThemeTokens;
}): ResolvedColorScope {
  ...
  return {
    ...
    style: {
      ...
      '--color-ornament': resolved.accent === 'transparent' ? 'transparent' : withOpacity(palette[resolved.accent], resolved.ornamentOpacity),
      '--radius-card': tokens.radii.card, '--radius-button': tokens.radii.button, '--radius-frame': tokens.radii.frame, '--radius-pill': tokens.radii.pill,
...
export function getBrandThemeTokensFromProfile(profile: BrandRenderProfile): BrandThemeTokens {
  return { brandKey: profile.id, themeId: profile.themeId ?? 'brochure', palette: profile.palette, radii: profile.radii };
}

/** Production V2 resolver: only backend-provided brand state enters this boundary. */
export function resolveColorSlotsFromProfile({ profile, theme, viewMode }: {
  profile: BrandRenderProfile; theme: ThemeDefinition; viewMode: ViewMode;
}): ResolvedColorSlots {
  ...
  return {
    page: resolve('page'), appChrome: resolve('appChrome'),
    sections: Object.fromEntries(theme.sectionOrder.map((sectionId) => [
      sectionId,
      resolve(theme.sectionConfigs[sectionId][viewMode].brandColorScopes?.[profile.id] ?? theme.sectionConfigs[sectionId][viewMode].colorScope),
