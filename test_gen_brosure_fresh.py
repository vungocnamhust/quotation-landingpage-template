...
# Force it to use the exact template we want
payload["template"] = "vietnam_luxury_brosure.html"
# Change quotation number so it doesn't clone from a published one
payload["quotationNumber"] = "QT-FRESH-TEST-BROSURE"
...
if response.status_code == 200:
    ...
    with open("test-brosure-fresh.html", "w") as f:
        f.write(html_res.text)
        
    pdf_res = client.get(f"/quotations/{q_id}/pdf?lang=en")
    with open("test-brosure-fresh-pdf.html", "w") as f:
        f.write(pdf_res.text)
        
    print("test-brosure-fresh.html and pdf generated.")
else:
